...
| `mutation` | You changed files — record what changed and the intent behind it |
| `outcome` | A previous action worked or failed — close the loop on past decisions |
...
Full-text search plus structured filters. Use this when you need specific context beyond what the briefing provides.
...
3. Prints a CLAUDE.md snippet with agent instructions
...
src/engram/
  ...
  store.py       — EventStore: SQLite + WAL mode + FTS5 full-text search
  ...
  briefing.py    — BriefingGenerator: summarizes events by type and time window
  formatting.py  — Compact single-line and JSON formatters
  cli.py         — Click CLI: init, post, query, briefing, status
  mcp_server.py  — FastMCP server: post_event, query, briefing, status tools
```

**Storage:** `.engram/events.db` — SQLite with WAL mode for concurrent access, FTS5 virtual table with auto-indexing triggers.

**Event schema:** Intentionally minimal — 6 fields: `id`, `timestamp`, `event_type`, `agent_id`, `content`, `scope`. No over-engineering. Fields that are "almost never populated" were cut during design.
...
**v1.0** (current) — Event store, CLI, MCP server, git bootstrap, briefing, FTS5 search

**v1.1** (next) — Passive observation via Claude Code hooks, event linking, CLAUDE.md auto-write, smarter briefings with dedup/staleness detection, garbage collection
